...
use layerd_core::{Document, HeadingLevel, MoveTarget, StructuralEditError};

fn doc(md: &str) -> Document {
    Document::parse(md.to_string()).unwrap()
}

fn node_ids(d: &Document) -> Vec<layerd_core::NodeId> {
...
fn structural_revision_mismatch_rejected() {
    ...
    d.replace_section_body(layerd_core::ReplaceSectionBody {
        node_id: a,
