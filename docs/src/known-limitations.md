...
current version of layerd. Limitations that affect data safety are listed
first.
...
make your changes, then re-open the file in layerd.
...
- Markdown source is **never silently modified** by layerd. If you open a
...
- Saved files are **standard UTF-8 Markdown**. They open correctly in any
  text editor, with no layerd-specific metadata.
